...
typedef Implementation_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Implementation(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Implementation_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(Implementation_DWork.HILReadTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Implementation:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Implementation -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(Implementation_DWork.HILReadTimebase_Task,
        (t_clock) Implementation_P.HILReadTimebase_Clock, 1000.0, -1)
        ;
      if (result == 0) {
